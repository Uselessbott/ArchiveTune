...
# ------------------------------------------------------------------------------
# Helper: find a Kotlin function body by signature using brace matching
# ------------------------------------------------------------------------------

def find_function_body(text: str, signature_regex: str) -> str | None:
...
"""
    ...
    # 4. In onCreate, add the line to set the tunnel provider with the extractor client.
    # Find the onCreate function body.
    sig_regex_oncreate = r'\bfun\s+onCreate\s*\([^)]*\)\s*\{'
    match = re.search(sig_regex_oncreate, content, re.MULTILINE)
    if not match:
        print("Error: Could not find onCreate function.", file=sys.stderr)
        sys.exit(1)
    start = match.start()
    ...
    while pos < len(content):
        ...
        print("Error: Could not find closing brace for onCreate.", file=sys.stderr)
        sys.exit(1)
